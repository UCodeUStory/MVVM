...
import ustory.com.mvvm.utils.FL;
...
public class ManagerActivity {
    ...
    private volatile static ManagerActivity mManager = null;
    ...
    private ManagerActivity(Context context) {
        mContext = context;
    }
     ...
     * @return
     */
    public static ManagerActivity init(Context applicationContext) {
        if (mManager == null) {
            synchronized (LOCK) {
                if (mManager == null) {
                    mManager = new ManagerActivity(applicationContext);
                }
            }
        }
        return mManager;
     ...
     * @param key        数据传输键值
     */
    public ManagerActivity openCrashHandler(String serverHost, String key) {
